...
export const RenderField = (props: {
  ...
  const [addActionButtonStatus, setAddActionButtonStatus] = useState(true);
  ...
  const valueChangeHandler = (value, keyname) => {
    setItem((oldItem) => {
      return { ...oldItem, [keyname]: value };
    });
  ...
  const fetchDate = async (value) => {
    const res = await fetch(`${SERVER_ENDPOINT}v1/calendarworkingday/list`, {
      method: "POST",
      headers: {
  ...
  const fetchSlotList = async (date, skuCode) => {
    const res = await fetch(`${SERVER_ENDPOINT}v1/slotcapacity/list`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  return (
    <Row>
      ...
      {Object.keys(tableHeaderObj).map(function (keyName) {
            ...
            >
              <Col>
                    ...
                    <View style={detailViewStyles.centeredView}>
                      <View style={detailViewStyles.modalView}>
                        ...
                        <Calendar
                          ...
                          onDayPress={(day) => {
                            valueChangeHandler(day.dateString, keyName);
                            fetchSlotList(day.dateString, item["skuCode"]);
                          }}
                ...
                <TouchableOpacity
                  ...
                  disabled={item["skuCode"] == undefined}
                >
        ...
        if (
          schema?.properties?.[keyName]?.displayType === "dropdown" ||
          keyName === "slotCode"
        ) {
          return (
            <Row
              ...
              <Col>
                <Picker
                  ...
                  onValueChange={(itemValue) => {
                    valueChangeHandler(itemValue, keyName);
                    if (keyName == "skuCode") {
                      fetchDate(itemValue);
                    }
                  }}
          ...
          >
            <Col>
              <TextInput
                ...
                value={item[keyName]}
                onChangeText={(text) => {
                  valueChangeHandler(text, keyName);
                }}
              ></TextInput>
            </Col>
          </Row>
